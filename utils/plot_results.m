...
for p = plots
  ...
  for k = 1:numel(measures), 
    ...
    for n=Ns,
      ...
      % Ploting the name of network
      % For the Proposed-ResNet-Mode, using:  leg{end+1} = sprintf('%s-%d',p,n); 
      % For the Proposed-ResNet-Mode, using:  leg{end+1} = sprintf('Original-ResNet-%d',n); %,p,n 
      leg{end+1} = sprintf('%d-layer ResNet',n); %,p,n 
      if numel(stats.train)>nEpoches, nEpoches = numel(stats.train); end
    end
    xlabel('Epoch', 'FontSize',10,'FontWeight','bold','Color','r') ;
    %ylabel(sprintf('%s', measures{k}));
    ylabel('Classification error (%)', 'FontSize',10,'FontWeight','bold','Color','r') ;
    title(p) ;
